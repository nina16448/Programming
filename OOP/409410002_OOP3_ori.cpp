...
class simple_link: public link {
    ...
    public:
        ...
        class simple_link_generator : public link_generator {
            ...
            protected:
                virtual link * generate(usint _id1, usint _id2) 
            ...
            if(update_table[{switch_ID, Mat_ID}] == 4){ //如果是第二次更新
                ...
                if(upd_flag){ //更新下一round
                    ...
                    for(auto packet: Upd_Round[{Mat_ID, upd_round[Mat_ID]}]){
                        // cout << "make ctrl packet To:" << packet.NodID << endl;
...
void Bellman_Ford(usint v, usint dstID){ //判斷每一個link會不會更新最短路徑
    for(auto links: graph){ //圖上的每一個link
        if(weight[links.node1] > weight[links.node2] + links.weight[v] || (weight[links.node1] == weight[links.node2] + links.weight[v] && road[{links.node1, dstID}][v] > links.node2)){
            ...
            if(v){
                if(!SPT[{road[{links.node1, dstID}][1], dstID}].empty())
                    SPT[{road[{links.node1, dstID}][1], dstID}].erase(links.node1);

                SPT[{links.node2, dstID}].insert(links.node1);
            }
        ...
        if(weight[links.node2] > weight[links.node1] + links.weight[v] || (weight[links.node2] == weight[links.node1] + links.weight[v] && road[{links.node2, dstID}][v] > links.node1)){ 
            ...
            if(v){
                if(!SPT[{road[{links.node2, dstID}][1], dstID}].empty())
                    SPT[{road[{links.node2, dstID}][1], dstID}].erase(links.node2);
                    
                SPT[{links.node1, dstID}].insert(links.node2);
            }
...
void Traverse(usint level, usint Dst_ID, vector <usint>& round){

    for(auto nei: SPT[{level, Dst_ID}]){
        round[nei] = round[level] + 1;
        Upd_Round[{Dst_ID, round[nei] - 1}].push_back({nei, level});


        Traverse(nei, Dst_ID, round);
    }
}
...
int main()
{
    ...
    //分配round
    vector <usint> round(nodes_num);
    for(auto dstID: dsts){
        round.clear();
        for(usint i = 0; i <= nodes_num; i++) round.push_back(0);
        // for(usint id = 0; id < nodes_num; id++) cout << round[id] << " ";
        // cout << "\n";

        Traverse(dstID, dstID, round);
        // for(int i = 0; i < nodes_num; i++){
        //     cout << "parent: " << i << "child: ";
        //     for(auto x: SPT[{i, dstID}]){
        //         cout << x << " ";
        //     }
        //     cout << endl;
        // }
