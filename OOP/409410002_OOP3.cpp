...
class SDN_controller : public node{
    ...
    map <usint, usint> upd_round; //map<DstID, round> DstID更新到第幾round

protected:
    SDN_controller(){} // it should not be used
    SDN_controller(SDN_controller &){} // it should not be used
    SDN_controller(usint _id) : node(_id){} // this constructor cannot be directly called by users
...
void SDN_switch::recv_handler(packet *p){
    ...
    if(p->type() == "SDN_data_packet"){ // the switch receives a packet from the other switch
        ...
        p3->getHeader()->setDstID(Con_ID); //傳回去給controller
...
void SDN_controller::recv_handler(packet *p){
    ...
    if(p->type() == "SDN_ctrl_packet"){ // the switch receives a packet from the controller
        ...
        // for(usint ID = 0; ID < getNodeID(); ID++)
        //     cout << update_table[{ID, Mat_ID}] << " ";
        // cout << endl;
        ...
        if(p3->getHeader()->getDstID() != getNodeID()){ //要傳給switch的
            ...
            update_table[{switch_ID, Mat_ID}] ++; //沒送過是0 建立initial tree為1 送出更新為3
            ...
            if(update_table[{switch_ID, Mat_ID}] == 4){ //如果是第二次更新
                ...
                for(usint ID = 0; ID < getNodeID(); ID++){ //看同一round的是否都更新完了
                    ...
                    // cout << "ID: " << ID << " state:" << update_table[{ID, Mat_ID}] << " flag: " << upd_flag << endl;  
                }
                if(upd_flag){ //更新下一round
                    upd_round[Mat_ID]++;
                    for(auto packet : Upd_Round[{Mat_ID, upd_round[Mat_ID]}]){
                        // cout << "Round:" << upd_round[Mat_ID] << " make ctrl packet To:" << packet.NodID << endl;
...
public:
    usint node1;
    usint node2;
    usint weight[2]; //邊的權重，0是舊的，1是新的

};

vector <link_info> graph; //用vector建圖
usint flag;
usint weight[10000]; //Bellman ford用來存原點到每個點的最短距離
map <pair <usint, usint>, usint[2]> road; //用map存每個node到終點的最短路徑的下一個點。key是點i跟各個終點的ID。value有兩項，第0項是存舊的圖的下一步，第1項是存新的圖的下一步
map <pair <usint, usint>, set <usint> > SPT[2]; // {node_ID, Dst_ID}對應到在終點為DstID時有哪些點的Next為這個node

void Bellman_Ford(usint v, usint dstID){ //判斷每一個link會不會更新最短路徑
    for(auto links : graph){ //圖上的每一個link
        if(weight[links.node1] > weight[links.node2] + links.weight[v] || (weight[links.node1] == weight[links.node2] + links.weight[v] && road[{links.node1, dstID}][v] > links.node2)){
            ...
            if(!SPT[v][{road[{links.node1, dstID}][v], dstID}].empty())
                SPT[v][{road[{links.node1, dstID}][v], dstID}].erase(links.node1);

            SPT[v][{links.node2, dstID}].insert(links.node1); //建SPT
        ...
        if(weight[links.node2] > weight[links.node1] + links.weight[v] || (weight[links.node2] == weight[links.node1] + links.weight[v] && road[{links.node2, dstID}][v] > links.node1)){
            ...
            if(!SPT[v][{road[{links.node2, dstID}][v], dstID}].empty())
                SPT[v][{road[{links.node2, dstID}][v], dstID}].erase(links.node2);

            SPT[v][{links.node1, dstID}].insert(links.node2);
...
void Init_Traverse(usint level, usint Dst_ID, vector <usint> &round, usint rnum, vector <pair <usint, usint> > &rm_leaf){
    for(auto nei : SPT[InitG][{level, Dst_ID}]){ //DFS每個child
        // cout << "Parent: " << level << " child: " << nei << endl;
        if(SPT[InitG][{nei, Dst_ID}].empty()){ //如果nei是葉子
            round[nei] = rnum; //更新nei的round
            rm_leaf.push_back({level, nei}); //拔點
            continue;
        }

        Init_Traverse(nei, Dst_ID, round, rnum, rm_leaf);
    }
}

void Upd_Traverse(usint level, usint Dst_ID, vector <usint> &round){ //DFS update tree的level

    for(auto nei : SPT[UpdG][{level, Dst_ID}]){
        round[nei] = min(round[level] + 1, round[nei]); //如果原本的round比較小就不更新

        Upd_Traverse(nei, Dst_ID, round);
    }
}
...
int main(){
    ...
    usint nodes_num, dsts_num, links_num, ID, insT, updT, dura;
    ...
    for(usint i = 0; i < nodes_num; i++){ //建立每個switch跟controller的連線
    ...
    //最短路徑
    for(usint i = 0; i < dsts_num; i++){ //i代表第幾個終點，把每個終點當成起點走到其他點
        for(usint ver = 0; ver < 2; ver++){ //跑舊的圖跟新的圖，0是舊的，1是新的
            for(usint j = 0; j < nodes_num; j++) //初始化，將起點到每個點的距離設為無限大
                weight[j] = 0x3f3f3f3f;
    ...
    //分配round
    vector <usint> round;
    vector <pair <usint, usint> > rm_leaf;
    for(auto dstID : dsts){
        round.clear();
        for(usint i = 0; i <= nodes_num; i++) round.push_back(0x3f3f3f3f); //初始化每個點的round都是最大值
        round[dstID] = 0;
        // for(usint id = 0; id < nodes_num; id++) cout << round[id] << " ";
        // cout << "\n";
        usint rnum = 1; //目前是第幾round
        while(!SPT[InitG][{dstID, dstID}].empty()){ //如果init樹的Dst還接著child
            rm_leaf.clear();
            Init_Traverse(dstID, dstID, round, rnum, rm_leaf); //找出所有葉子
            for(auto leaf : rm_leaf) SPT[InitG][{leaf.first, dstID}].erase(leaf.second); //拔點
            rnum++;
        }
        // cout << "init round:\n";
        ...
        Upd_Traverse(dstID, dstID, round); //離Dst幾步就第幾round更新

        for(usint id = 0; id < nodes_num; id++){
            if(id == dstID) continue;
            Upd_Round[{dstID, round[id] - 1}].push_back({id, road[{id, dstID}][UpdG]}); //第幾round要更新那些點
        }
        // cout << "upd round:\n";
        // for(auto x: round) cout << x << " ";
        // cout << endl;
        ...
        // for(usint id = 0; id < nodes_num; id++){
        //     cout << "DST=" << dstID << " round=" << round[id] << " ID=" <<  id << " next:" << road[{id, dstID}][1] << endl;
        // }
    }

    for(auto dstID : dsts){ //初始ctrl packet
        for(usint id = 0; id < nodes_num; id++){
            if(id == dstID) continue;
            ctrl_packet_event(con_id, id, dstID, road[{id, dstID}][0], insT, "init\n");
            // ctrl_packet_event(con_id, id, dstID, road[{id, dstID}][1], updT);
        ...
        for(auto packet : Upd_Round[{dstID, 0}]){ //送出第0round更新的封包

            // cout << "Dst:" << dstID << " round: 1 nodId:" << packet.NodID << " ActID:" << packet.ActID << endl;
            // cout << "Round:" << 0 << " make ctrl packet To:" << packet.NodID << endl;
            ctrl_packet_event(con_id, packet.NodID, dstID, packet.ActID, updT, "Upd first\n");
        }
    }
